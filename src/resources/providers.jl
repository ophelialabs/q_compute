    # Page title and optional disclaimer 
    "title" => "Providers",
    "disclaimer" => "",
...
# ============================================================================
# Rendering Code - Display as Cards
# ============================================================================
...
begin
    ...
    <style>
        ...
        .card {
            background: var(--pluto-background);
...
begin
    page_data = @htl("""
    <div>
        <h2 class="section-title">Quantum Providers</h2>
        <div class="card-grid">
            $(map(page.output.frontmatter["providers"]) do provider
                @htl("""
                <div class="card">
                    <img class="card-img" src="$(provider["img"])" alt="$(provider["name"])">
                    <div class="card-content">
                        <span class="card-category">$(provider["category"])</span>
                        <h3 class="card-title">$(provider["name"])</h3>
                        <p class="card-description">$(provider["description"])</p>
                        <a href="$(provider["url"])" target="_blank" class="card-link">Learn More →</a>
                    </div>
                </div>
                """)
            end)
        ...
        <div class="card-grid">
            $(map(page.output.frontmatter["learning_resources"]) do resource
                @htl("""
                <div class="card">
                    <img class="card-img" src="$(resource["img"])" alt="$(resource["name"])">
                    <div class="card-content">
                        <span class="card-category">$(resource["category"])</span>
                        <h3 class="card-title">$(resource["name"])</h3>
                        <p class="card-description">$(resource["description"])</p>
                        <a href="$(resource["url"])" target="_blank" class="card-link">Learn More →</a>
                    </div>
                </div>
                """)
            end)
...
page_data    # Page title and optional disclaimer 
    "title" => "Providers",
    "disclaimer" => "",
